...
public class TextPromptScreen extends BaseScreen {
    ...
    protected void buildWidgets() {
        int il = innerLeft();
        int iw = innerWidth();
        this.inputField = new TextFieldWidget(this.font, il, this.panelTop + 54, iw, UiConstants.BTN_HEIGHT, new StringTextComponent(this.inputLabel));
        ...
        if (compact) {
            ...
            this.addButton(new StyledButton(il, btnY + UiConstants.BTN_STRIDE, iw, UiConstants.BTN_HEIGHT, new StringTextComponent("粘贴"), StyledButton.Variant.GHOST, button -> this.inputField.setValue(PlatformServices.client().readClipboard())));
            ...
            this.addButton(new StyledButton(this.panelLeft + 126, btnY, 100, UiConstants.BTN_HEIGHT, new StringTextComponent("粘贴"), StyledButton.Variant.GHOST, button -> this.inputField.setValue(PlatformServices.client().readClipboard())));
            this.addButton(new StyledButton(this.panelLeft + 234, btnY, 128, UiConstants.BTN_HEIGHT, new StringTextComponent("取消"), StyledButton.Variant.SECONDARY, button -> this.minecraft.setScreen(this.parent)));
        }
    }
    ...
    protected void renderContent(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks) {
        ...
        this.font.draw(matrixStack, this.inputLabel, (float) il, (float) (this.panelTop + 42), UiPalette.TEXT_MUTED);
