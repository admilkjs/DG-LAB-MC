...
public class WaveformImportScreen extends Screen {
    ...
    protected void init() {
        ...
        this.nameField = new TextFieldWidget(this.font, left + 18, top + 52, panelWidth - 36, 20, new StringTextComponent("名称"));
        this.nameField.setMaxLength(80);
        this.nameField.setValue("pulse".equals(this.importMode) ? "导入的 pulse 波形" : "导入的 HEX 波形");
        this.children.add(this.nameField);

        this.descriptionField = new TextFieldWidget(this.font, left + 18, top + 86, panelWidth - 36, 20, new StringTextComponent("说明"));
        this.descriptionField.setMaxLength(120);
        this.children.add(this.descriptionField);

        this.rawInputField = new TextFieldWidget(this.font, left + 18, top + 120, panelWidth - 36, 20, new StringTextComponent("原始输入"));
        ...
        if (compact) {
            ...
            this.addButton(new StyledButton(left + 18, top + 188, panelWidth - 36, 20, new StringTextComponent("粘贴"), StyledButton.Variant.GHOST, button -> this.rawInputField.setValue(PlatformServices.client().readClipboard())));
            this.addButton(new StyledButton(left + 18, top + 212, panelWidth - 36, 20, new StringTextComponent("取消"), StyledButton.Variant.SECONDARY, button -> this.minecraft.setScreen(this.parent)));
        } else {
            ...
            this.addButton(new StyledButton(left + 134, top + 164, 108, 20, new StringTextComponent("粘贴"), StyledButton.Variant.GHOST, button -> this.rawInputField.setValue(PlatformServices.client().readClipboard())));
            this.addButton(new StyledButton(left + 250, top + 164, 152, 20, new StringTextComponent("取消"), StyledButton.Variant.SECONDARY, button -> this.minecraft.setScreen(this.parent)));
        }
    }
