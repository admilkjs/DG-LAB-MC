...
public final class ForgeCommandRegistrar {
    ...
    private static Iterable<String> collectRuleTokens() {
        ...
        for (cn.admilk.dglabweb.rule.RuleDefinition rule : AppServices.get().getConfig().rules) {
    ...
    private static Iterable<String> collectWaveformTokens() {
        Set<String> tokens = new LinkedHashSet<String>();
        for (cn.admilk.dglabweb.wave.WaveformDefinition waveform : AppServices.get().getConfig().waveforms) {
