...
import cn.admilk.dglabweb.client.ClientCommandRouter;
...
public class RuleEngine {
    ...
    public RuleEngine(ConfigRepository configRepository, DeviceSessionManager sessionManager) {
        ...
        try {
            ...
            for (RuleDefinition rule : config.rules) {
                ...
                if (groupId != currentGroupId) {
                    if (dispatchMatchingGroup(config, currentGroup, context, now)) {
                        matchedAnyGroup = true;
                        return;
                    }
                    ClientCommandRouter.reportRuleRowContinue(currentGroupId + 1);
                    currentGroup.clear();
                    currentGroupId = groupId;
                }
            ...
            if (!matchedAnyGroup) {
                ClientCommandRouter.reportRuleNoMatch();
            }
    ...
    public synchronized void preview(RuleDefinition rule) {
        ...
        try {
            ...
            dispatch(config, rule, waveform, createPreviewContext(rule.trigger));
        } catch (IOException ignored) {
        }
    }
    ...
    private void dispatch(AppConfig config, RuleDefinition rule, WaveformDefinition waveform, RuleEventContext context) {
        switch (rule.channel) {
            case BOTH:
                applyEvent(this.stateA, config.strength.channelA, config.strength.maxStrengthA, waveform, rule, context);
                applyEvent(this.stateB, config.strength.channelB, config.strength.maxStrengthB, waveform, rule, context);
                break;
            case B:
                applyEvent(this.stateB, config.strength.channelB, config.strength.maxStrengthB, waveform, rule, context);
                break;
    ...
    private void applyEvent(ChannelRuntime runtime, AppConfig.ChannelStrengthProfile profile, int configuredMax, WaveformDefinition waveform, RuleDefinition rule, RuleEventContext context) {
    ...
    private boolean dispatchMatchingGroup(AppConfig config, List<RuleDefinition> rules, RuleEventContext context, long now) {
        ...
        for (RuleDefinition rule : rules) {
            ...
            dispatch(config, rule, executable.waveform, context);
            cooldowns.put(rule.id, Long.valueOf(now));
            matchedRuleNames.add(labelForDebug(rule));
            matched = true;
        }
        if (matched) {
            ClientCommandRouter.reportRuleRowMatched(rowIndex, matchedRuleNames);
            ClientCommandRouter.reportRuleStop(rowIndex);
        }
        return matched;
    }
    ...
    private int resolveDeviceMax(DeviceChannel channel) {
        ...
        int reportedMax = channel == DeviceChannel.B ? snapshot.maxStrengthB : snapshot.maxStrengthA;
